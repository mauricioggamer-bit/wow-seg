<script lang="ts">
  ...
  const COMPONENT_ROWS = [
    ...
    { key: 'classValue', label: 'Clase', weight: 'fijo', calc: (s: StrategicValueResult) => s.classValue, raw: (s: StrategicValueResult) => s.classValue, desc: 'Suma de todas las ventajas asignadas a la clase del personaje, en cualquier categoría (no solo "General").' },
    { key: 'raceValue', label: 'Raza', weight: 'fijo', calc: (s: StrategicValueResult) => s.raceValue, raw: (s: StrategicValueResult) => s.raceValue, desc: 'Suma de todas las ventajas asignadas a la raza del personaje, en cualquier categoría (no solo "General").' },
    { key: 'professionExtraVentajas', label: 'Profesión (ventajas adicionales)', weight: 'fijo', calc: (s: StrategicValueResult) => s.professionExtraVentajas, raw: (s: StrategicValueResult) => s.professionExtraVentajas, desc: 'Ventajas asignadas a la profesión fuera del índice "General" — se suman sobre el valor base del juego (fila "Profesiones" arriba).' },
    { key: 'personajeVentajas', label: 'Personaje (ventajas propias)', weight: 'fijo', calc: (s: StrategicValueResult) => s.personajeVentajas, raw: (s: StrategicValueResult) => s.personajeVentajas, desc: 'Suma de las ventajas asignadas directamente a este personaje (no heredadas de clase/raza/profesión).' },
    { key: 'warbandVentajas', label: 'Warband (ventajas)', weight: 'fijo', calc: (s: StrategicValueResult) => s.warbandVentajas, raw: (s: StrategicValueResult) => s.warbandVentajas, desc: 'Suma de las ventajas asignadas al warband del personaje.' },
    { key: 'taskGlobalVentajas', label: 'Tareas (ventaja global)', weight: 'fijo', calc: (s: StrategicValueResult) => s.taskGlobalVentajas, raw: (s: StrategicValueResult) => s.taskGlobalVentajas, desc: 'Ventajas asignadas de forma global a "Tareas" como tipo de entidad (aplican una vez por personaje).' },
    ...
    { key: 'taskValue', label: 'Tareas (puntos manuales)', weight: 'fijo', calc: (s: StrategicValueResult) => s.taskValue, raw: (s: StrategicValueResult) => s.taskValue, desc: 'Suma de los puntos cargados a mano en cada tarea del personaje (distinto de la ventaja global de Tareas).' },
  ] as const
