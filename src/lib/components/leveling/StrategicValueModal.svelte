<script lang="ts">
  ...
  const COMPONENT_ROWS = [
    ...
    { key: 'classValue', label: 'Clase', weight: 'fijo', calc: (s: StrategicValueResult) => s.classValue, raw: (s: StrategicValueResult) => s.classValue, desc: 'Valor fijo según la clase del personaje.' },
    { key: 'raceValue', label: 'Raza', weight: 'fijo', calc: (s: StrategicValueResult) => s.raceValue, raw: (s: StrategicValueResult) => s.raceValue, desc: 'Valor fijo según la raza del personaje.' },
  ...
  let starBand = $derived(strategic ? [...STAR_THRESHOLDS].find(t => strategic.totalScore >= t.min) : null)
</script>
