...
<div class="rc-wrap" style="position:relative">
  <svg viewBox="0 0 {SVG_W} {svgH}" class="rc-svg">
    ...
    {#each data.races as race, ri}
      ...
      <text x={LABEL_W - 4} y={HEADER_H + ri * (CELL + GAP) + CELL / 2 + 1} text-anchor="end" dominant-baseline="middle" fill="#e0e0e0" font-size="6.5">
        {info.icon} {race}
      </text>
...
<style>
  ...
  .rc-tooltip { position: fixed; transform: translate(-50%, -100%); background: var(--bg-panel, #1a1a1a); border: 1px solid var(--gold, #d4af37); border-radius: var(--r-sm); padding: 4px 8px; z-index: 1100; pointer-events: none; white-space: nowrap; box-shadow: 0 4px 16px rgba(0,0,0,0.5); font-size: 0.55rem; color: var(--text-secondary, #aaa); }
</style>
