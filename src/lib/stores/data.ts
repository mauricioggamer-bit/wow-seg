...
function createDataStore() {
  ...
  const store = {
    ...
    renamePersonaje(oldName: string, newName: string): boolean {
      ...
      update(d => {
        ...
        if (d.strategicConfig?.values) {
          const prefix = `personaje:${oldName}:`
          const newValues: Record<string, number> = {}
          for (const [key, val] of Object.entries(d.strategicConfig.values)) {
            newValues[key.startsWith(prefix) ? `personaje:${trimmed}:${key.slice(prefix.length)}` : key] = val
          }
          d.strategicConfig.values = newValues
        }
        saveData(d)
        return { ...d }
      })
    ...
    updateNivel(nombre: string, nivel: number) {
      update(d => {
        d.personajes = d.personajes.map(p => p.nombre === nombre ? { ...p, nivel } : p)
        saveData(d)
        return { ...d }
      })
    },
...
addTarea(nombrePersonaje: string, tarea: { nombre: string; tipoContenido?: TipoContenido; contenidoExpansion?: string; contenidoDificultad?: string; tipo?: string; cooldown?: string; prioridad?: number; tiempo_min?: number; expansion?: string; nivelRecomendado?: number; recompensa?: string; puntos?: number }) {
      update(d => {
        ...
        const nuevaTarea = {
          ...
          nivelRecomendado: tarea.nivelRecomendado,
